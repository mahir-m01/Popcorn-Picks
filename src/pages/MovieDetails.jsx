...
function MovieDetails() {
  ...
  const backdropStyle = backdropUrl ? {
    ...
    alignItems: 'center',
    padding: isMobile ? '1.2rem 0.2rem' : '3rem 1rem',
  } : {
    minHeight: '100vh',
    display: 'flex',
    justifyContent: 'center',
  ...
  return (
    <div className="home" style={backdropStyle}>
      <div className="movie-card" style={{
        ...
        boxShadow: '0 8px 24px 0 rgba(40,22,54,0.22)',
        alignItems: isMobile ? 'center' : 'flex-start',
      }}>
        <div className="movie-poster" style={{minWidth: isMobile ? 100 : 150, maxWidth: isMobile ? 180 : 180, flex: isMobile ? '0 0 100px' : '0 0 150px', display: 'flex', justifyContent: 'center'}}>
          <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} style={{width: '100%', maxWidth: isMobile ? 120 : 150, borderRadius: isMobile ? 10 : 16, boxShadow: '0 4px 24px 0 rgba(0,0,0,0.25)'}} />
        </div>
        <div className="movie-info" style={{textAlign: 'left', flex: 1, display: 'flex', flexDirection: 'column', gap: isMobile ? '0.5rem' : '0.7rem', fontSize: isMobile ? '0.98rem' : '1.08rem'}}>
          <h2 style={{marginBottom: 4, fontSize: isMobile ? '1.3rem' : '2.1rem', color: '#fff', fontWeight: 700, letterSpacing: '0.01em'}}>{movie.title}</h2>
          <p style={{margin: 0, color: '#b5a7f7', fontSize: isMobile ? '1rem' : '1.08rem'}}><b>Original Title:</b> {movie.original_title}</p>
          <div style={{display: 'flex', flexWrap: 'wrap', gap: isMobile ? '0.7rem 1rem' : '1.2rem 1.5rem', margin: '0.2rem 0'}}>
          ...
          <p style={{margin: '0.7rem 0', color: '#e0e0e0', fontSize: isMobile ? '0.98rem' : '1.08rem', lineHeight: 1.7}}><b>Overview:</b> {movie.overview}</p>
